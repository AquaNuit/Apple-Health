...
struct HealthMetricFamilyView: View {
  ...
  var body: some View {
    if route == .sleep {
      SleepV2OverviewPage(store: store, selectedDate: selectedDateBinding)
  ...
  private var heroRows: [HealthSummaryRow] {
    ...
    case .sleep:
      return [
        ...
        HealthSummaryRow("Alarm", value: "Not available with Apple Health", source: .unavailable("Alarm requires WHOOP BLE connection"), systemImage: "bell"),
      ]
    case .recovery:
  ...
  private var alarmRowSource: HealthDataSource {
    .unavailable("Alarm requires WHOOP BLE connection")
  }

...
private let timelineLogger = Logger(subsystem: "com.goose.swift", category: "ActivityTimeline")
...
extension GooseAppModel {
    ...
    rustStartupQueue.async { [weak self] in
      ...
      DispatchQueue.main.async {
        ...
        case .success(let repaired):
          if repaired > 0 {
            timelineLogger.info("orphan_zero_frame.repaired sessions=\(repaired)")
          }
        case .failure(let error):
          timelineLogger.warning("orphan_zero_frame.repair_failed: \(error.localizedDescription)")
        }
      }
    }
  }
  ...
  func activityMetricsByName(sessionID: String) -> [String: [String: Any]] {
    do {
      ...
      timelineLogger.warning("metric.load.failed: \(sessionID) \(String(describing: error))")
      return [:]
    }
  }
  ...
  func attachActivityMetrics(_ metrics: [[String: Any]]) {
    ...
    do {
      ...
      timelineLogger.debug("activity.metrics.store.ok inserted=\(inserted) existing=\(existing) requested=\(metrics.count)")
    } catch {
      timelineLogger.warning("activity.metrics.store.failed count=\(metrics.count): \(error.localizedDescription)")
    }
  }
  ...
  // Overnight guard directory methods removed — no longer applicable with HealthKit ingestion.

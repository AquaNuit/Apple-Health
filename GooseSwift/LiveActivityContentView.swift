...
struct LiveActivityContentView: View {
  ...
  var body: some View {
    ZStack {
      ...
      if let countdownValue {
        ...
        FitnessSummaryView(activity: session.selectedActivity, session: session, healthKit: healthKit, locationTracker: locationTracker) {
          dismiss()
        }
  ...
  private func primaryAction() {
    ...
    if session.isActive {
      session.pause(heartRate: healthKit.latestHeartRateBPM)
      if session.selectedActivity.usesGPS {
  ...
  private func startWorkoutNow() {
    ...
    WorkoutLiveActivityController.shared.start(
      activity: session.selectedActivity,
      session: session,
      heartRate: healthKit.latestHeartRateBPM,
  ...
  private func endActivity() {
    ...
    controlsLocked = false
    let endedAt = Date()
    session.end(now: endedAt, heartRate: healthKit.latestHeartRateBPM)
    locationTracker.stop()

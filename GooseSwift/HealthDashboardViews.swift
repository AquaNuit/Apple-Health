    ...
    ScrollView {
      LazyVStack(alignment: .leading, spacing: 18) {
        ...
        VStack(spacing: 8) {
          ...
          HealthInfoRow(row: HealthSummaryRow("Heart rate 1D", value: store.heartRateTimelineStatus, source: store.heartRateHourlyRanges.isEmpty ? .unavailable("Apple Health heart-rate") : .live("Apple Health heart-rate"), systemImage: "heart"))
  ...
  var body: some View {
    List {
      ...
      Section("Packet-Derived Inputs") {
        ...
        HealthInfoRow(row: HealthSummaryRow("Latest HR", value: store.latestHeartRateSummary(bpm: model.healthKit.latestHeartRateBPM, source: "Apple Health", updatedAt: model.healthKit.latestHeartRateDate), source: model.healthKit.latestHeartRateBPM == nil ? .unavailable("Apple Health HR unavailable") : .live("Apple Health HR"), systemImage: "heart"))
        if !store.latestHeartRateProvenanceSummary(source: "Apple Health").isEmpty {
          HealthInfoRow(row: HealthSummaryRow("HR provenance", value: store.latestHeartRateProvenanceSummary(source: "Apple Health"), source: .live("latestHeartRateProvenanceSummary()"), systemImage: "doc.text.magnifyingglass"))
        }

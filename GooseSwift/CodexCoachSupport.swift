...
enum CodexLocalToolContext {
  ...
  private static func loadStats(
    ...
    return [
      ...
      "health_monitor": healthStore.healthMonitorSnapshots(
        restingHeartRateEstimateBPM: nil,
        restingHeartRateEstimateSampleCount: 0,
        restingHeartRateEstimateUpdatedAt: nil,
        restingHeartRateEstimateSource: "apple_health"
      ).map(snapshotPayload),
      ...
      "live_heart_rate": healthStore.latestHeartRateSummary(
        bpm: appModel.healthKit.latestHeartRateBPM,
        source: "apple_health",
        updatedAt: appModel.healthKit.latestHeartRateDate
      ),
      "recovery": [
  ...
  private static func getCaptureSessions(appModel: GooseAppModel) -> [String: Any] {
    [
      ...
      "device": devicePayload(appModel.healthKit),
    ]
  }
  ...
  private static func devicePayload(_ healthKit: GooseHealthKitManager) -> [String: Any] {
    [
      "connection": healthKit.isAuthorized ? "authorized" : "unauthorized",
      "active_device_name": "Apple Health",
      "last_sync_at": isoString(healthKit.lastSyncAt)
    ]
  }

...
struct MoreAboutView: View {
  ...
  var body: some View {
    List {
      ...
      Section("Runtime") {
        MoreInfoRow(title: "Data Source", value: model.healthKit.isAuthorized ? "Apple Health (Authorized)" : "Apple Health (Not Authorized)", systemImage: "heart.fill", status: model.healthKit.isAuthorized ? .ready : .pending)
        MoreInfoRow(title: "Hello", value: model.helloSummary, systemImage: "hand.wave", status: model.helloSummary.hasPrefix("GET_HELLO") || model.helloSummary.contains("HealthKit") ? .ready : .pending)
      }

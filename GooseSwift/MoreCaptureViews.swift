...
struct MoreCaptureView: View {
  ...
  var body: some View {
    List {
      Section("Session") {
        ...
        MoreInfoRow(title: "Live Notifications", value: store.liveNotificationCaptureSummary(), systemImage: "dot.radiowaves.left.and.right", status: .pending)
        MoreInfoRow(title: "Data Source", value: "Apple Health", systemImage: "heart.fill", status: model.healthKit.isAuthorized ? .ready : .pending)
        Button {
      ...
      Section("Overnight Guard") {
        ...
        MoreInfoRow(
          title: "Range Polls",
          value: "HealthKit mode — range polls not applicable",
        ...
        HStack {
          ...
          .disabled(true)  // Overnight guard requires BLE
  ...
  private var selectedDeviceStatus: MoreStatusKind {
    model.healthKit.isAuthorized ? .ready : .pending
  }

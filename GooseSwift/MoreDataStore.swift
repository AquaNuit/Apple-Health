...
final class MoreDataStore: ObservableObject {
  ...
  func routeStatus(model: GooseAppModel) -> MoreRouteStatus {
    MoreRouteStatus(
      profile: OnboardingProfileSnapshot().hasRequiredDetails ? .ready : .pending,
      device: model.healthKit.isAuthorized ? .ready : .pending,
      connectionLab: model.healthKit.isAuthorized ? .ready : .pending,
  ...
  func liveNotificationCaptureSummary() -> String {
    return "HealthKit mode — BLE notification capture not applicable"
  }

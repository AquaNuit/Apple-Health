...
struct OnboardingView: View {
  ...
  private var content: some View {
    ...
    case .bluetooth:
      OnboardingPermissionStep(
        ...
        bodyText: "Goose reads heart rate, HRV, sleep, active energy, and respiratory rate from Apple Health to power your dashboard.",
        details: [
          "Heart rate and HRV for recovery scores",
          "Sleep analysis for sleep scores",
          "Active energy for strain calculation",
        ],
    ...
    case .connect:
      // Connect step removed — no BLE pairing needed with Apple Health
      EmptyView()
  }
  ...
  private func requestBluetoothAccess() {
    // Bluetooth is no longer needed — skip this step
    bluetoothPermissionResolved = true
    model.recordUIAction("onboarding.bluetooth.skipped")
    moveForward()
  }
  ...
  private func shouldSkip(_ candidate: OnboardingStep) -> Bool {
    ...
    case .bluetooth:
      return true  // Bluetooth no longer needed
    case .notifications:
  ...
  private var bluetoothStateIsResolved: Bool {
    true  // Bluetooth is no longer needed with Apple Health
  }
